...
PersonalDialog::PersonalDialog(QWidget *parent,QTcpSocket *_tcp,QString _hisname,QString _selfName,  QMap<QString,PersonalDialog*> *list) :
    QWidget(parent),mytcp(_tcp),oppositeName(_hisname),selfName(_selfName),mylist(list),
    ui(new Ui::p2p)
{
    ui->setupUi(this);
    this->setWindowTitle(tr(oppositeName.toUtf8()+"     私聊窗口"));

    setNameText(oppositeName);
    ...
    mylist->insert(oppositeName,this);//pdList的插入！
...
void PersonalDialog::on_pushButton_send_2_clicked()//聊天记录导出
{
    ...
    if (!dir.exists(dir_str))
    {
        ...
        qDebug() << "新建目录是否成功" << res;
        if(!res){
            QMessageBox::information(this,"错误","目录："+dir_str+" 创建失败！",QMessageBox::Ok);
    ...
    {
        QMessageBox::information(this,"提示", "<font color = #00CDCD> 聊天文件读取失败！ < / font>");
        return;
    }
    ...
    chat.close();
    QMessageBox::information(this,"提示", "<font color = #000000> 聊天文件导出成功！ < / font>");// QMessageBox::information(NULL 有问题！！
